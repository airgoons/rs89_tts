...
def parse_redstrike_hierarchy(buildfile_path, bson_data):
    ...
    factions_raw = data_raw["VASSAL.build.GameModule"]["VASSAL.build.module.PieceWindow"][0]["VASSAL.build.widget.TabWidget"]["VASSAL.build.widget.TabWidget"]
    
    factions = []
    for faction_raw in factions_raw:
        factions.append(Faction(faction_raw, bson_data))
...
def publish_json(factions, json_path):
...
if __name__ == "__main__":
    ...
    json_path = "{0}.json".format(vmod_path)

    extract_vassal_file(vmod_path, vmod_temp)
    bson_data = parse_bson(bson_path)
    factions = parse_redstrike_hierarchy(buildfile_path, bson_data)
    cleanup(vmod_temp)

    publish_json(factions, json_path)
    print("DONE!")
